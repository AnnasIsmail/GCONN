...
function MyStoreContainer(props) {
  ...
  React.useEffect(() => {
    ...
    fetch(
      `https://gconn-api-node-js.vercel.app/accountsOwner/${cookies.Cr787980}`
    )
      .then((response) => response.json())
      .then((res) => {
    ...
    // if(render.current === 1){
    fetch(
      `https://gconn-api-node-js.vercel.app/getTransactionByIDSeller/${cookies.Cr787980}`
    )
      .then((response) => response.json())
      .then((res) => {
        res.data.forEach((data, index) => {
          if (
            ...
            data.status === "Waiting for Seller to Respond" ||
            data.status === "Seller Already Sent Credentials" ||
            data.status === "Waiting for Seller to Send Credentials" ||
            data.status ===
              "Waiting for the seller to accept the cancellation" ||
            data.status === "Waiting for admin's decision"
          ) {
      ...
      render: () => (
        <>
          {MyProductData.length !== 0 ? (
            ...
            <NoData
              description="There are no accounts that you have marketed yet"
              goto="/choosegamesell/sellaccountvalorant"
              button="Go To Sell Account"
            />
          )}
        </>
      ),
