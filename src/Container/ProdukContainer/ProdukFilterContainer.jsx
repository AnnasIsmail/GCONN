...
function ProdukFilterContainer(props) {
    ...
    function fetchData() {
      ...
      fetch(`https://gconn-api-node-js.vercel.app/accounts`)
        ...
        .then((res) => {
          ...
          fetch(
            `https://gconn-api-node-js.vercel.app/favorites/${cookies.Cr787980}`
          )
            .then((response) => response.json())
            .then((res) => {
      ...
      if (props.page === "market-main-container") {
        ...
        allAccount.map((data, index) => {
          if (data.idSeller !== cookies.Cr787980) {
            let favorite = allFavorite.find(
              (index) => index.idAccount == data._id
            );
  ...
  return <div className="produk-container">{content}</div>;
}
