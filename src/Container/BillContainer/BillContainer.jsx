...
function BillContainer(props) {
  ...
  function getTransaction() {
    fetch(`https://gconn-api-node-js.vercel.app/getTransaction/${id}`)
      ...
      .then((res) => {
        ...
        fetch(
          `https://gconn-api-node-js.vercel.app/accountDetail/${res.data.idAccount}`
        )
          .then((response) => response.json())
          .then((res) => {
            setAccount(res.data);
  ...
  function checkPayment() {
    fetch(
      `https://gconn-api-node-js.vercel.app/checkPayment/${id}/${transaction.response_midtrans.order_id}`
    )
      .then((response) => response.json())
      .then((res) => {
        if (res.status === 200) {
    ...
    <div className="payment-container">
      ...
      {loading ? (
        ...
        <>
          ...
          >
            Username:
            <h5>{transaction.credentials.username}</h5>
          </p>
          <p
            style={{
              margin: 0,
          ...
          >
            Password:
            <h5>{transaction.credentials.password}</h5>
          </p>
            ...
            <>
              {transaction.paymentMethod !== "Mandiri Bill Payment" ? (
                ...
                >
                  ...
                  <h5>
                    {transaction.response_midtrans.va_numbers.map(
                      (data) => data.va_number
                    )}
                  </h5>
                </p>
          ...
          {cookies.Cr787980 === transaction.idUser ? (
            <>
              {transaction.status === "Waiting For Payment" ? (
                <div className="button-container">
                  <Button onClick={() => changeStatus("Transaction Failed")}>
                    Cancel Transaction
                  </Button>
                ...
                <div className="button-container">
                  <Button loading={loadingChat} onClick={goToChat}>
                    Chat Seller
                  </Button>
                  <Button
                    onClick={() =>
                      changeStatus(
                        "Waiting for the seller to accept the cancellation"
                      )
                    }
                  >
                ...
                "Waiting for Seller to Send Credentials" ? (
                <div className="button-container">
                  <Button loading={loadingChat} onClick={goToChat}>
                    Chat Seller
                  </Button>
              ...
              <div className="on-button report">
                <Button
                  loading={loadingChatReport}
                  color="red"
                  onClick={goToChatAdmin}
                >
                  Report Admin
                </Button>
              </div>
            ...
            <>
              {transaction.status === "Waiting For Payment" ? (
                <div className="button-container">
                  <Button loading={loadingChat} onClick={goToChatCustomer}>
                    Chat Customer
                  </Button>
                  <Button onClick={() => changeStatus("Transaction Failed")}>
                    Cancel Transaction
                ...
                <div className="button-container">
                  ...
                  <Button
                    onClick={() =>
                      changeStatus("Waiting for Seller to Send Credentials")
                    }
                  >
                    Accept Transaction
                  </Button>
                ...
                <div className="button-container">
                  ...
                  <Button onClick={() => setModalCredentials(true)}>
                    Send Credentials
                  </Button>
                </div>
              ) : transaction.status === "Seller Already Sent Credentials" ? (
