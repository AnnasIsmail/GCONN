...
function DetailChat(props) {
  ...
  React.useEffect(() => {
    ...
    socket.current.on("getUsers", (data) => {
      const checkOnline = data.filter(
        (dataFind) => dataFind.idUser === profile._id
      );
    ...
    fetch("https://gconn-api-node-js.vercel.app/updateChat", {
      ...
      .then((json) => {
        if (json.status === 200) {
          ...
          const getIdUser = getUserOnline.filter(
            (data) => data.idUser === profile._id
          );
  ...
  function read() {
    ...
    const readFalse = dataChat.content.filter(
      (dataFilter) =>
        dataFilter.from !== cookies.Cr787980 && dataFilter.read === false
    );
          ...
          {dataChat.content.map((dataContent, index) => (
            <div key={index} className="container-Chat-P">
              {dataContent.from === cookies.Cr787980 ? (
                <Fragment key={index}>
                  <span className="content-chat-right" key={index}>
                    <p
                      onMouseLeave={() => displayLeave(index)}
                      onMouseEnter={() => displayEnter(index)}
                    >
                      {" "}
                      {dataContent.read === true ? (
                        <Icon
                          name="check"
                          size="small"
                          color="blue"
                ...
                dataContent.from !== undefined ? (
                <Fragment key={index}>
                  <span className="content-chat-left" key={index}>
                    <p
                      onMouseLeave={() => displayLeave(index)}
                      onMouseEnter={() => displayEnter(index)}
                    >
                      {dataContent.message}
                      <small>{dataContent.time}</small>
                    </p>
                  </span>
                  <Moment id={`ago${index}`} toNow>
                    {dataContent.dateTime}
                  </Moment>
                  <Moment
                    id={`detail${index}`}
